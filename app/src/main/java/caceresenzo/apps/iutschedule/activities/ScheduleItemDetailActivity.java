...
public class ScheduleItemDetailActivity extends AppCompatActivity {
	 ...
	 * @param items
	 *            {@link List} of item for the section. This list will be processed to only display one with {@link Utils#validate(String...) validated} content.
	 */
	private void addCardItem(String title, AboutItem... items) {
		List<AboutItem> aboutItems = new ArrayList<>();
		
		for (AboutItem item : items) {
			if (Utils.validate(item.getTitle())) {
				aboutItems.add(item);
			}
		}
